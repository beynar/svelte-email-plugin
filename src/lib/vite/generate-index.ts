...
export interface GenerateIndexOptions {
	 ...
	 * Defaults to `'svelte-email-plugin'`.
	 */
	importSource?: string;
}

/** Header comment marking the generated file as machine-written. */
const HEADER = '// AUTO-GENERATED by svelte-email-plugin/vite — do not edit.';
...
function serializeNode(node: TreeNode, depth: number): string {
	...
	// Files claim their key first; a file↔file clash gets a numeric suffix.
	for (const leaf of [...node.leaves].sort((a, b) => cmp(a.stem, b.stem))) {
		const base = safeIdentifier(toCamelCase(tokenize(leaf.stem)), 'email');
		let key = base;
		let suffix = 2;
		while (used.has(key)) key = `${base}${suffix++}`;
		used.add(key);
		children.push({ rawName: leaf.stem, key, kind: 'leaf', leaf });
	}
	// Folders come second: a clash with a sibling file takes an `_` prefix, leaving
	// the file's key unchanged (folder names are unique, so folders never clash).
	for (const [rawName, child] of [...node.folders].sort((a, b) => cmp(a[0], b[0]))) {
		const base = safeIdentifier(toCamelCase(tokenize(rawName)), 'email');
		let key = base;
		while (used.has(key)) key = `_${key}`;
		used.add(key);
		children.push({ rawName, key, kind: 'folder', node: child });
	}
...
export function generateIndex(sveltePaths: string[], options?: GenerateIndexOptions): string {
	const importSource = options?.importSource ?? 'svelte-email-plugin';
